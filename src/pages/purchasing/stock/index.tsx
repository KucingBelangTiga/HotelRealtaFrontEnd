...
const Index = () => {
    const { stocks, total } = useSelector((state: any) => state.stockState)
    ...
    useEffect(() => { dispatch(GetStockRequest(page)) }, [dispatch, page])
                ...
                <LayoutMaster>
                    <div className="flex flex-col items-center min-h-screen">
                        ...
                        <Dialog header={isEdit ? 'Edit' : 'Create'} visible={visible} style={{ width: '50vw' }} onHide={() => setVisible(false)}>
                            {isEdit ? <EditStock stockName={stockName} stockDescription={stockDescription} stockQuantity={stockQuantity} stockReorderPoint={stockReorderPoint} stockUsed={stockUsed} stockScrap={stockScrap} stockSize={stockSize} stockColor={stockColor} stockId={stockId} /> : <AddStock stockId={total + 1} />}
                        </Dialog>
