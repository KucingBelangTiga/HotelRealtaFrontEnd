...
import { GetAllUserAccountRequest, DeleteUserAccountRequest } from "@/src/redux/action/payment/userAccountAction";
...
export default function Index() {
  ...
  useEffect(() => {
    ...
    dispatch(GetAllUserAccountRequest(payload));
    setLoading(true);
    setRefresh(false);
          ...
          <>
            {!loading ? (
              ...
              <div className="m-4 min-h-screen">
                ...
                <table className="w-full border-collapse my-2 rounded-lg border border-gray-200 bg-white text-left text-sm text-gray-500 ">
                  ...
                  <tbody className="divide-y divide-gray-100 border-t border-gray-100">
                    {(userAccounts.items || []).map((item: any) => (
                      <tr className="hover:bg-gray-50 py-2 h-3" key={item.usacAccountNumber}>
                        <td className="px-4 py-3">{item.usacAccountNumber}</td>
                        <td className="px-4 py-3">{item.usacEntity ? item.usacEntity.bank.bankName : null}</td>
                        <td className="px-4 py-3">{item.usacSaldo}</td>
                        <td className="px-4 py-3">{item.usacType}</td>
                        <td className="px-4 py-3">
                          <div className="flex justify-end gap-4 relative">
                            <button x-data="{ tooltip: 'Edite' }" className="text-coldBlue hover:bg-coldBlue hover:text-white p-2 rounded-full" onClick={() => displayKebabMenu(item.usacAccountNumber)}>
                            ...
                            {item.usacAccountNumber === id && kebabMenu && (
                              <div className="absolute top-10 border-coldBlue text-coldBlue w-96 z-30">
                                <ul className="bg-white  absolute right-3  border-coldBlue border-solid border-2 rounded-md text-center">
                                  <li>
                                    <Edit id={item.usacAccountNumber} setRefresh={setRefresh} />
                                  </li>
                                  <li>
                                    <Toast ref={toast} className="mt-14" />
                                    <ConfirmDialog />
                                    <button onClick={() => handleDelete(item.usacEntityId, item.usacUserId)} className="p-3 w-full bg-white text-red-600 hover:bg-red-600 hover:text-white font-semibold">
                                      Delete
                                    </button>
                                  </li>
                                </ul>
                ...
                {userAccounts?.meta && <Pagination currentPage={userAccounts.meta.currentPage} totalPages={userAccounts.meta.totalPages} setpage={setpage} page={page} items={userAccounts.items} />}
              </div>
            )}
          </>
        </LayoutPayment>
