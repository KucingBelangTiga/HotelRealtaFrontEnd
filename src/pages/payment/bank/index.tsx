...
import { GetAllBankRequest, DeleteBankRequest } from "@/src/redux/action/payment/bankAction";
...
export default function Index() {
  ...
  useEffect(() => {
    ...
    dispatch(GetAllBankRequest(payload));
    setLoading(true);
    setRefresh(false);
              ...
              <div className="m-4 min-h-screen">
                ...
                <table className=" w-full border-collapse my-5 rounded-lg border border-gray-200 bg-white text-left text-sm text-gray-500 ">
                  ...
                  <tbody className="divide-y divide-gray-100 border-t border-gray-100">
                    {(banks.items || []).map((item: any) => (
                      <tr className="hover:bg-gray-50 py-2 h-3" key={item.bankEntityId}>
                        <td className="px-4 py-3">{item.bankCode}</td>
                        <td className="px-4 py-3">{item.bankName}</td>
                        <td className="px-4 py-3">
                          <div className="flex justify-end gap-4 relative">
                            <button x-data="{ tooltip: 'Edite' }" className="text-coldBlue hover:bg-coldBlue hover:text-white p-2 rounded-full" onClick={() => displayKebabMenu(item.bankEntityId)}>
                            ...
                            {item.bankEntityId === id && kebabMenu && (
                              <div className="absolute top-10 border-coldBlue text-coldBlue w-96 z-30">
                                <ul className="bg-white  absolute right-3  border-coldBlue border-solid border-2 rounded-md text-center">
                                  <li>
                                    <Edit id={item.bankEntityId} setRefresh={setRefresh} />
                                  </li>
                                  <li>
                ...
                {banks.meta && <Pagination currentPage={banks.meta.currentPage} totalPages={banks.meta.totalPages} setpage={setpage} page={page} items={banks.items} />}
              </div>
            )}
          </>
        </LayoutPayment>
